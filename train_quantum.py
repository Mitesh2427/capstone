...
for subj in SUBJECTS:
    ...
    for cfg in CONFIGS:
        ...
        if acc > best_acc:
            ...
            plt.savefig(os.path.join(PLOTS_DIR, f"quantum_confusion_subject_{subj}.png"))
            plt.close()
            ...
            plt.plot([0, 1], [0, 1], 'k--')
            plt.legend()
            plt.savefig(os.path.join(PLOTS_DIR, f"quantum_roc_subject_{subj}.png"))
            plt.close()

            plt.plot(val_history)
            plt.title("Validation Curve")
            plt.savefig(os.path.join(PLOTS_DIR, f"quantum_train_curve_subject_{subj}.png"))
            plt.close()
    ...
    torch.save(best_state, os.path.join(SAVE_DIR, f"quantum_subject_{subj}.pt"))

    results.append({
        "subject": subj,
        "model": "EEGNet+TCN+Quantum",
        "accuracy": best_acc,
        "kappa": best_kappa,
